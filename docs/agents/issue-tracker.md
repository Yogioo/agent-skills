...
**Remote:** `Yogioo/agent-skills`（`git@github.com:Yogioo/agent-skills.git`）

## Conventions

- **Create an issue**: `gh issue create --title "..." --body "..."`。多行 body 使用 `--body-file` 或 heredoc。
...
## Spec / epic 约定（`/to-spec`）

- **Epic（parent spec）**：一个 GitHub issue 承载完整 spec（Problem / Solution / User Stories / Decisions）。**不要**打 `ready-for-agent`（避免被 AFK 当作可执行工单）。body 底部用 task list 链到子 issue，例如 `- [ ] #12`。
- **Phase / 子 ticket**：独立 issue，body 顶部 `Part of #<epic>`；可执行子单打 `ready-for-agent`。
- **阻塞（AFK `--source gh`）**：子 issue body 用 Markdown task list 表达依赖，例如 `- [ ] #11` 表示被 #11 阻塞；前置关闭或勾选后该 issue 才 ready。
- **优先级（AFK）**：标签 `P0`~`P4`（数值越小越优先）；无 P 标签默认 P2。
...
创建一个 GitHub issue（epic + 子 issue 按需）。
...
`.beads/` 曾为本地 Dolt issue 库；**canonical tracker 已迁至 GitHub**。新 spec 请用 `gh issue create`，勿再写 `.scratch/` 或 `bd create` 作为发布目标。
