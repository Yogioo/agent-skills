...
`run-task.mjs` 的执行流程与摘要契约不绑定具体 CLI。一次 agent turn 通过 **runner** 完成。默认只跑一次执行 turn（`review: false`）；开启 `review: true` 时再跑一次审查 turn（审查端直接改进，不循环）。`gitCommit: true` 时执行端/审查端自行 commit；否则不提交。提交格式不在 exec-review 范围。

默认值读 `~/.afk/config.json` 的 `execReview` 分区（见 [config.md](config.md)）；`runner` 必填，一处都不给就报错。
